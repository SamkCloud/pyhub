...
class FileError(Exception):
	# used if files was not written 
	pass

class Pyfig:
	"""contains attributes and methods for the configuration file provided."""
	
	def __init__(self, config_file=None, individual_seperator = separator):
	...
	def commit(self, filename,individual_seperator = separator):
		"""write the config to a filename"""
		try:
			f = open(filename, 'w')
			for item in self.config:
				line = item + individual_seperator + self.config[item]+config_seperator
				f.writelines(line)
			f.close()	
		except KeyError:
			raise FileError("I was unable to write the config file")
...
if __name__ == "__main__":
	...
	print pyfig.config
	#pyfig.change('chiave2','nuovoval')
	#pyfig.commit('ldapnew.conf')
